...
export function registerChannelTools(server: McpServer): void {
  server.registerTool(
    ...
    async () => {
      ...
      try {
        const rest = getRest();
        const channels = (await rest.get(Routes.guildChannels(getActiveGuildId()))) as APIChannel[];
        return {
          content: [
            {
              type: 'text' as const,
  ...
  server.registerTool(
    ...
    async ({
      ...
      try {
        ...
        const channel = (await rest.post(Routes.guildChannels(getActiveGuildId()), {
          body,
          reason,
        })) as APIChannel;
        return {
  ...
  server.registerTool(
    ...
    async ({ positions, reason }) => {
      ...
      try {
        const rest = getRest();
        await rest.patch(Routes.guildChannels(getActiveGuildId()), {
          body: positions,
          reason,
        });
        return {
