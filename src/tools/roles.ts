...
export function registerRoleTools(server: McpServer): void {
  server.registerTool(
    ...
    async () => {
      ...
      try {
        const rest = getRest();
        const roles = (await rest.get(Routes.guildRoles(getActiveGuildId()))) as APIRole[];
        return {
          content: [
            {
              type: 'text' as const,
  ...
  server.registerTool(
    ...
    async ({ name, permissions, color, hoist, mentionable, reason }) => {
      ...
      try {
        ...
        const role = (await rest.post(Routes.guildRoles(getActiveGuildId()), {
          body,
          reason,
        })) as APIRole;
        return {
  ...
  server.registerTool(
    ...
    async ({ role_id, name, permissions, color, hoist, mentionable, reason }) => {
      ...
      try {
        ...
        if (hoist !== undefined) body.hoist = hoist;
        if (mentionable !== undefined) body.mentionable = mentionable;
        const role = (await rest.patch(Routes.guildRole(getActiveGuildId(), role_id), {
          body,
          reason,
        })) as APIRole;
        return {
  ...
  server.registerTool(
    ...
    async ({ role_id, reason }) => {
      ...
      try {
        const rest = getRest();
        await rest.delete(Routes.guildRole(getActiveGuildId(), role_id), { reason });
        return {
          content: [
            {
              type: 'text' as const,
  ...
  server.registerTool(
    ...
    async ({ positions, reason }) => {
      ...
      try {
        const rest = getRest();
        const roles = (await rest.patch(Routes.guildRoles(getActiveGuildId()), {
          body: positions,
          reason,
        })) as APIRole[];
        return {
